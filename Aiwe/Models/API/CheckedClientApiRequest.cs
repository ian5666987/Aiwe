...
namespace Aiwe.Models.API {
  public class CheckedClientApiRequest {
    ...
    public static Dictionary<string, ApiRequestType> RequestDict = new Dictionary<string, ApiRequestType> {
      { "GETMANY", ApiRequestType.SelectMany }, { "GET", ApiRequestType.Select },
      { "POST", ApiRequestType.Create }, { "PUT", ApiRequestType.Update },
    ...
    public string CreateDeleteString() {
      if (string.IsNullOrWhiteSpace(TableName) || RequestType != ApiRequestType.Delete || Id == null)
        return null;
      ...
      try {
        ...
        foreach (var item in items) {
          ...
          if (!string.IsNullOrWhiteSpace(valueStr) && valueStr.EqualsIgnoreCaseTrim("NULL"))
            ...
            object value = null;
            value = KeyInfo.ExtractValueAsObjectForWebApi(dataType, valueStr, refDtNow, 
              TableName, columnName, isTimeStamp, isTimeStampFixed, timeStampShift, isAutoGenerated, tableColumnNamePairs);
    ...
    public string CreateUpdateString(out List<SqlParameter> pars) {
      pars = null;
      if (string.IsNullOrWhiteSpace(TableName) || RequestType != ApiRequestType.Update || Id == null)
        return null;

      StringBuilder sb = new StringBuilder("UPDATE ");
      sb.Append(string.Concat("[", TableName, "] SET "));
    ...
    public string CreateInsertIntoString(out List<SqlParameter> pars) { //Yet the method's name is "insert into" following SQL command convention --bad naming, bad connection
      pars = null;
      if (string.IsNullOrWhiteSpace(TableName) || RequestType != ApiRequestType.Create) //and the request type is CREATE somemore
        return null;

      StringBuilder sb = new StringBuilder("INSERT INTO ");
      sb.Append(string.Concat("[", TableName, "] VALUES("));
    ...
    public string CreateQueryStringSingle() {
      if (string.IsNullOrWhiteSpace(TableName) || RequestType != ApiRequestType.Select || Id == null)
        return null;
    ...
    public string CreateQueryString() {
      if (string.IsNullOrWhiteSpace(TableName) || RequestType != ApiRequestType.SelectMany)
        return null;
