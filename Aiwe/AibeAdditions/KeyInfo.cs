...
namespace Aibe.Models.Core {
  public partial class KeyInfo {
    ...
    public object ExtractValueAsObject(FormCollection collections, DateTime refDtNow, bool filterStyle = false, 
      ...
      if (filterStyle) {
        if (DataType.EqualsIgnoreCase(Aibe.DH.StringDataType) || DataType.EqualsIgnoreCase(Aibe.DH.CharDataType))
          return value;
          ...
          string dtStr = value;
          string timeStr = AddKeyName.EndsWith(Aibe.DH.FromName) ? //get the time string
            collections[PureKeyName + Aibe.DH.FilterTimeAppendixFrontName + Aibe.DH.FromName] :
            collections[PureKeyName + Aibe.DH.FilterTimeAppendixFrontName + Aibe.DH.ToName];
      ...
      switch (DataType) {
        case Aibe.DH.StringDataType:
          string picStr = collections[PureKeyName + Aibe.DH.CreateEditPictureLinkAppendixName]; //Non filter time
        ...
        case Aibe.DH.DateTimeDataType:
          DateTime valDt; //this is going to be quite tricky
          string dtStr = value;
          string timeStr = collections[PureKeyName + Aibe.DH.CreateEditTimeAppendixName]; //Non filter time
